            ...
            <tr key={appt.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
              ...
              <td className="px-4 py-3.5 pr-4">
                <div className="flex items-center gap-1.5">
                  {!doctor?.viewOnly && ['scheduled','confirmed'].includes(appt.status) && appt.patientId && (
                    <button
                  ...
                  {!doctor?.viewOnly && appt.source === 'booking_link' && !appt.patientId && !patientPhoneSet.has(normalizePhone(appt.patientPhone)) && (
                    <button
                  ...
                  {!doctor?.viewOnly && <button onClick={() => onRemove(appt.id)}
          ...
          <div className="space-y-5">
            ...
            <div className="flex gap-3 pt-1">
              ...
              {isWhatsAppApiConnected(doctor) ? (
                <button onClick={handleSendViaApi} disabled={apiSending || !remindPhone.trim()}
            ...
            {isWhatsAppApiConnected(doctor) && (
              <div className="flex items-center justify-between -mt-2">
                {apiResult && (
                  <p className={`text-xs ${apiResult.success ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    {apiResult.success ? '✓ Sent successfully' : `✗ ${apiResult.error}`}
                  </p>
                )}
                <button onClick={handleWhatsApp} className="text-xs text-gray-400 dark:text-gray-500 hover:underline ml-auto">
