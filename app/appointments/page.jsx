...
function ApptActions({ appt, router, onEdit, onRemind, onRemove, patientPhoneSet }) {
  return (
    <div className="flex flex-wrap items-center gap-1.5">
      ...
      <button onClick={() => onEdit(appt)} className="text-xs text-primary-600 dark:text-primary-400 hover:underline font-medium px-1">
        Change
      </button>
      <button onClick={() => onRemove(appt.id)} className="text-gray-400 hover:text-red-500 transition-colors p-1 rounded">
        <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function AppointmentTable({ appointments, formatDate, formatTime, router, onEdit, onRemind, onRemove, patientPhoneSet = new Set(), muted = false }) {
  return (
    <div className={`bg-white dark:bg-gray-800 rounded-xl border shadow-sm overflow-hidden ${muted ? 'border-gray-200 dark:border-gray-600 opacity-80' : 'border-gray-100 dark:border-gray-700'}`}>

      {/* ── Mobile cards (hidden on sm+) ─────────────────────────────────────── */}
      <div className="sm:hidden divide-y divide-gray-50 dark:divide-gray-700">
        {appointments.map(appt => (
          <div key={appt.id} className="px-4 py-4 space-y-2.5">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{appt.patientName}</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                  {formatDate(appt.date)} · {formatTime(appt.time)} · {appt.durationMinutes}min
                </p>
                {appt.reason && <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{appt.reason}</p>}
              </div>
              <Badge label={appt.status} color={STATUS_COLOR[appt.status] ?? 'gray'}/>
            </div>
            <ApptActions appt={appt} router={router} onEdit={onEdit} onRemind={onRemind} onRemove={onRemove} patientPhoneSet={patientPhoneSet}/>
          </div>
        ))}
      </div>
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1.5">
                Enter number or leave blank to open WhatsApp picker.
              </p>
            </div>
