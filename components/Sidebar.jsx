...
export default function Sidebar({ unreadCount = 0 }) {
  ...
  return (
    <aside className="w-64 bg-white border-r border-gray-100 flex flex-col flex-shrink-0 h-screen sticky top-0">
      ...
      <div className="px-4 py-4 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            {doctor?.clinicName ? (
              <>
                <p className="text-gray-900 font-bold text-sm leading-tight truncate">{doctor.clinicName}</p>
                <p className="text-gray-400 text-xs truncate">ClinicCRM</p>
              </>
            ) : (
              <span className="text-gray-900 font-bold text-lg">ClinicCRM</span>
            )}
          </div>
        </div>
      </div>
        ...
        {navItems.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={active
                ? 'flex items-center gap-3 px-3 py-2.5 rounded-lg bg-blue-50 text-blue-700 font-semibold text-sm'
                : 'flex items-center gap-3 px-3 py-2.5 rounded-lg text-gray-600 hover:bg-gray-50 hover:text-gray-900 font-medium text-sm transition-colors'
              }
            >
              {item.icon}
              {item.label}
