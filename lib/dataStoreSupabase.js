...
// Kept in sync via onAuthStateChange purely as a fallback (see below) — not
// the primary doctorId source, since Supabase's session read is async and
// isn't populated yet on a fresh app start.
let _session = null
...
// The doctorId a given call should operate on. `session.id` in the local
// session object is ALWAYS the owning doctor's id — for a doctor session
// that's their own id; for a receptionist session it's the doctor they're
// linked to (buildDoctorProfileFromRow's shape is spread into both). This is
// read synchronously from localStorage rather than from the async Supabase
// session on purpose: on a fresh app start, hooks fire their first
// dataStore.subscribe()/getAll() call before supabase.auth.getSession()'s
// promise has resolved, so relying on `_session` here meant doctorId was
// still null on that first call — subscriptions silently never attached,
// which showed up as "data doesn't refresh on app start" for SB accounts.
