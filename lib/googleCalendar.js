...
// Called on login to restore the connection flag from the saved profile
export function restoreGoogleCalendarConnection(connected) {
...
function toGCalFollowUpEvent(followup) {
  return {
    summary:     `Follow-up: ${followup.patientName}`,
    description: followup.note || 'Scheduled follow-up',
    start: { date: followup.dueDate },  // all-day event
    end:   { date: followup.dueDate },
...
export async function createFollowUpEvent(followup) {
  const event = await calendarFetch('events', {
    method: 'POST',
    body:   JSON.stringify(toGCalFollowUpEvent(followup)),
  })
  return event.id
}

export async function updateFollowUpEvent(googleEventId, followup) {
  await calendarFetch(`events/${googleEventId}`, {
    method: 'PATCH',
    body:   JSON.stringify(toGCalFollowUpEvent(followup)),
  })
