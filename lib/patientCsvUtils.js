...
export function importPatientsCsv(csvText) {
  ...
  rows.forEach((r, idx) => {
    ...
    // Primary key: normalised phone + name. Falls back to name+DOB if no phone.
    const key = phone
      ? `${phone}__${firstName.toLowerCase()}__${lastName.toLowerCase()}`
      : `${firstName.toLowerCase()}__${lastName.toLowerCase()}__${dob}`
