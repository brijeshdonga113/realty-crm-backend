...
/**
 * Returns an inline <script> string that runs synchronously before first paint,
 * reading the saved colorTheme from localStorage and immediately setting CSS
 * variables — prevents a flash of the default teal color on non-teal themes.
 */
export function getThemeScript() {
  // Embed the full theme map as a JSON literal so the script is self-contained
  const map = {}
  THEMES.forEach(t => { map[t.key] = t.vars })
  return `
(function(){
  try {
    var key = localStorage.getItem('colorTheme') || 'teal';
    var themes = ${JSON.stringify(map)};
    var vars = themes[key] || themes['teal'];
    var root = document.documentElement;
    Object.keys(vars).forEach(function(p){ root.style.setProperty(p, vars[p]); });
  } catch(e){}
})();
`.trim()
}
