 ...
 *
 * Firestore structure:
 *   clinics/{doctorId}/{collection}/{docId}
 *   clinics/{doctorId}/patients/{patientId}/visits/{visitId}  ← nested subcollection
 *
 * API:
...
function getCollectionRef(doctorId, collPath) {
  ...
  let ref = fsCollection(db, 'clinics', doctorId)
...
const firestoreStore = {
  ...
  async getMeta(key_) {
    ...
    if (!doctorId) return null
    const { doc, getDoc } = await import('firebase/firestore')
    const ref  = doc(db, 'clinics', doctorId, 'meta', key_)
    const snap = await getDoc(ref)
