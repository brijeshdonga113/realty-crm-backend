...
function getCollectionRef(doctorId, collPath) {
  ...
  // Start from the doctor's document: users/{doctorId}
  const parts = collPath.split('/')
  let ref = fsDoc(db, 'users', doctorId)

  for (let i = 0; i < parts.length; i++) {
    if (i % 2 === 0) {
      ref = fsCollection(ref, parts[i])  // collection segment
    } else {
      ref = fsDoc(ref, parts[i])         // document segment
    }
  }
  return ref
}
...
const firestoreStore = {
  ...
  subscribe(collPath, callback) {
    const doctorId = getCurrentDoctorId()
    if (!doctorId) return () => {}
    return onSnapshot(getCollectionRef(doctorId, collPath), snap => {
...
// Use Firestore when the project ID env var is present, otherwise localStorage
